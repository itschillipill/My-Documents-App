            ...
            child:
                !isSelecting
                    ? AppBar(
                      ...
                      actions:[
                           _buildSortButton(context, sortOptions, isReverse, onPressed:(i, so) => setState(() {
                            isReverse = i;
                            sortOptions = so;
                           }),),
                              if(!folder.isVirtual)
                              MenuActions(actions:[
                                (
                                  Rename$FolderAction(context: context, folder: folder).call,
                                  context.l10n.rename,
                                ),
                                (
                                  Delete$FolderAction(context: context, folder: folder).call,
                                  context.l10n.delete,
                                ),
                               ]),
                    ],)
                    : AppBar(
                      ...
                      actions: [
                        IconButton(
                          onPressed: (){
                           Delete$DocumentAction(documentsIds: selectedDocumentsIds.toList(), context: context).call();
                            resetSelecting();
                          },
                        ...
                        IconButton(
                          onPressed: () {
                            Share$DocumentAction(documents: context.deps.documentsCubit.getDocumentsByIds(selectedDocumentsIds.toList()), context: context).call();
                            resetSelecting();
                          },
...
Widget _buildSortButton(BuildContext context, SortOptions sortOptions, bool isReverse, {required Function(bool, SortOptions) onPressed}){
return IconButton(
