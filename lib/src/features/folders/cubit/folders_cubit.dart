...
class FoldersCubit extends Cubit<FoldersState> {
 ...
 @protected
  Future<void> loadData() async {
    emit(FoldersState.processing(folders: state.folders, message: 'Loading data...'));
    try {
      final folders = await dataSource.getAllFolders();
      emit(FoldersState.processing(folders: folders, message: 'Data loaded successfully.'));
    } catch (e,s) {
      emit(FoldersState.failed(folders: state.folders, message:"Error loading data", error: e, stackTrace: s));
    }finally{
  ...
  Future<void> addFolder(Folder folder) async {
    emit(FoldersState.processing(folders: state.folders, message: 'Adding folder...'));
    try {
     Folder newFolder= await dataSource.insertFolder(folder);
      emit(FoldersState.processing(folders: [...?state.folders, newFolder], message: 'Folder added successfully.'));
    } catch (e,s) {
      emit(FoldersState.failed(folders: state.folders, message:"Error adding folder", error: e, stackTrace: s));
    }finally{
      emit(FoldersState.idle(folders: state.folders));
    }
  }
  ...
  emit(FoldersState.processing(folders: state.folders, message: 'Deleting folder...'));
  try {
    ...
    emit(FoldersState.processing(
        folders: updatedFolders, message: 'Folder deleted successfully.'));
  } catch (e, s) {
    emit(FoldersState.failed(
  ...
  emit(FoldersState.processing(folders: state.folders, message: 'Updating folder...'));
  try {
    ...
    emit(FoldersState.processing(
        folders: updatedFolders, message: 'Folder updated successfully.'));
  } catch (e, s) {
    emit(FoldersState.failed(
        folders: state.folders,
